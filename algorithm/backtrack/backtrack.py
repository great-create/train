...
def main():
    ...
    # 每節點最便宜進入邊與 footprint（=最小進入+服務），及最小回倉成本：上界剪枝用
    min_in = [None] * N
    foot = [0] * N
    for j in range(1, N):
        m = None
    ...
    path = [0]
    memo = {}                      # (visited_mask, last) -> 已知到達的最短時間（支配剪枝）

    def bound(time_used):
        # 上界：扣掉必要回倉後，用剩餘時間依「獎勵大→小」貪婪裝填（footprint 為每點下界成本）
    ...
    def dfs(cur, mask, time_used, reward):
        ...
        for j in order:
            ...
            key = (nmask, j)
            prev = memo.get(key)
            if prev is not None and prev <= nt:    # 已有同集合同結尾、且時間更短 → 被支配，剪枝
                continue
            memo[key] = nt
            path.append(j)
