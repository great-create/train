...
public:
    ...
    int getTotalPrice() {
        ...
        for (auto &p : selectedItems)
            sum += p.second.getQuantity() * p.second.getProduct().getPriceCents();
        return sum;
    }

    void returnChange(int cents) {
        cout << "Change Returned:\n";
        cout << "25¢ x " << cents / 25 << endl; cents %= 25;
        cout << "10¢ x " << cents / 10 << endl; cents %= 10;
        cout << "5¢  x " << cents / 5 << endl;
    }
